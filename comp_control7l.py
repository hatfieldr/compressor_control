...
T4=0.0
T5=0.0
global P1,P1_zero,P1_cal,P2,P3,P4,P5,P2_zero,P2_cal,P3_zero,P3_cal,P4_zero,P4_cal
P1 = 0.0
P2=0.0
P3=0.0
P4=0.0
...
#set up MQTT client for data logging and connect to broker
broker_log_address = config.broker_log_address
broker_log_port = config.broker_log_port
usermane_log = config.username_log
password_log = config.password_log
client_log = mqtt.Client("P1_log") #create new instance
client_log.username_pw_set(usermane_log, password=password_log) #set username and password
client_log.connect(broker_log_address, broker_log_port) #connect to broker
print("connecting to logging broker")

#set up subscribe topics for logging
#client_log.subscribe("emon/emonpi/T1")
def on_connect_log(client_log, userdata, flags, rc):
    print("Client log Connected with RC "+str(rc))
...
def log_data():
    global mp,mt,fc T1,T2,T3,T4,T5,P1,P2,P3,P4
    #print("log data")

    client_log.publish("emon/H2 Compressor/Input Temp",str(T1))
    client_log.publish("emon/H2 Compressor/Stage 1 Temp",str(T2))
    client_log.publish("emon/H2 Compressor/Stage 2 Temp",str(T3))
    client_log.publish("emon/H2 Compressor/High Pressure Temp",str(T4))
    client_log.publish("emon/H2 Compressor/Input Pressure",str(P1))
    client_log.publish("emon/H2 Compressor/Stage 1 Pressure",str(P2))
    client_log.publish("emon/H2 Compressor/Stage 2 Pressure",str(P3))
    client_log.publish("emon/H2 Compressor/Output Pressure",str(P4))
    if car_data_info_event.is_set():
        client_log.publish("emon/Car Data/Tank temp",str(mt))
        client_log.publish("emon/Car Data/Tank Pressure",str(mp))
...
root.after(400,log_data)

client_log.on_connect = on_connect_log #attach function to callback

client_log.loop_start() #start the loop for MQTT logging
